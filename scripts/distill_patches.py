...
# -*- coding: utf-8 -*-
"""
补丁蒸馏工具：拆分大补丁为独立小补丁，再合并目标项目中存在的文件到 need_merge.diff
...
from diff_parser import detect_format
...
def main():
    ...
    # 自动检测格式并获取解析器
    parser = detect_format(diff_text)
    parser_name = parser.__class__.__name__
    print("检测到补丁格式: {}".format(parser_name))

    # 解析
    file_blocks, file_order = parser.parse(diff_text)
    ...
    for idx, filepath in enumerate(file_order, start=1):
        ...
        change_type, file_type = parser.detect_file_info(blocks)

        change_stats[change_type] = change_stats.get(change_type, 0) + 1
        file_stats[file_type] = file_stats.get(file_type, 0) + 1

        # 写出小补丁文件
        patch_filename = parser.make_patch_filename(idx, filepath)
        ...
        # Git format-patch: 附加 commit 元数据
        commit_meta = parser.extract_commit_metadata(blocks)
        if commit_meta:
            info["commit"] = commit_meta
